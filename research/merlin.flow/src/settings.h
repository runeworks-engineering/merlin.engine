...
struct Settings {
	...
	int pressure_iteration = 6;
	int visco_iteration = 15;
	float overRelaxation = 1.0;

	//Boundary Volume dimensions
	glm::vec3 bb = glm::vec3(200, 200, 50);
	...
	Uniform<float> viscosity						= Uniform<float>("u_viscosity", 40.0);
	//Uniform<float> artificialViscosityMultiplier	= Uniform<float>("u_artificialViscosityMultiplier", 47 * 0.001);
	Uniform<float> artificialViscosityMultiplier	= Uniform<float>("u_artificialViscosityMultiplier", 500 * 0.001);
	...
	inline void setConstants(ShaderBase& shader) {
		...
		shader.setConstVec3("cst_boundaryMin", bb*glm::vec3(-0.5, -0.5, -0.01) + glm::vec3(0, 0, 0.0));
		shader.setConstVec3("cst_boundaryMax", bb*glm::vec3(0.5, 0.5, 0.99) + glm::vec3(0, 0, 0.0));
